...
DEFINE FIELD IF NOT EXISTS created_at ON interpreta TYPE datetime DEFAULT time::now();
...
DEFINE FIELD IF NOT EXISTS created_at       ON menciona TYPE datetime DEFAULT time::now();
...
DEFINE FIELD IF NOT EXISTS created_at    ON relacionado_a TYPE datetime DEFAULT time::now();
"#;
